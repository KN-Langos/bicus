...
| `<expr>` | Expression. This can be any constant expression. |
| `<pat> or <pat> or ...` | List of possible patterns, This requires that all subpatterns bind the same names to the same types. |
| `.<tag?>{ x, y: <pat>, .. }` or `Name { x, y, .. }` | Struct | Binds fields of a struct-type value. Supports "rest" syntax with `..`. Allows for an optional tag, in which case it will work just like `.{ .. } @ .<tag>`|
